...
// import {IStorage} from "../resolvers/profiles/IStorage.sol";
    ...
    ReverseClaimer
{
    ...
    bytes32 private constant ETH_NODE =
        ...
        0x5d51f7be71608b15a476a7755e3eba23f59b90507c31a9d029cb98afee8536f8; //mtd
    bytes32 private constant ETH_LABELHASH =
        // 0x4f5b812789fc606be1b3b16908db13fc7a9adf7ca72641f84d75b47069d3d7f0; //eth
        0x1d762f12d73c53a2dfc38a124ae63612f55d0effc2adb139742105e93020fbbf; //mtd
    ...
    function registerAndWrapETH2LD(
        ...
        bytes32 node = _makeNode(ETH_NODE, labelhash);
            ...
            //them vao
        DATASTORAGE.setInfo(wrappedOwner,string.concat(label, ".mtd"),registrarExpiry,uint256(node));
    ...
    function renew(
        uint256 tokenId,
        uint256 duration,
        string calldata nameRenew
    ) external onlyController returns (uint256 expires,address owner) {
        ...
        try registrar.ownerOf(tokenId) returns (address registrarOwner) {
            if (
                ...
                return (registrarExpiry,registrar.ownerOf(tokenId));
            }
        } catch {
            return (registrarExpiry,registrar.ownerOf(tokenId));
        }
        ...
        DATASTORAGE.updateExpire(owner,nameRenew,registrarExpiry);
        return (registrarExpiry,owner);
    }
    ...
    function setSubnodeRecord(
        ...
        require(address(reverseRegistrar) != address(0),"have not set reverseRegistrar yet");
        bytes32 labelhash = keccak256(bytes(label));
        node = _makeNode(parentNode, labelhash);
        _checkCanCallSetSubnodeOwner(parentNode, node);
        _checkFusesAreSettable(node, fuses);
        ...
        _setReverseRecordSubname(label, resolver, msg.sender,parentNode);
        string memory parentName = nameFromHash[parentNode];
        DATASTORAGE.setInfo(owner,string.concat(label, ".",parentName),expiry,uint256(node));
    }
    function setReverseRegistrar(address _reverseRegistrar) public onlyOwner {
        reverseRegistrar = IReverseRegistrar(_reverseRegistrar);
    }
    function _setReverseRecordSubname(
        string memory label,
        address resolver,
        address owner,
        bytes32 parentNode
    ) internal {
        require(address(reverseRegistrar) != address(0),"have not set reverseRegistrar");
        reverseRegistrar.setSubNameForAddr(
            msg.sender,
            owner,
            resolver,
            label,
            parentNode,
            nameFromHash[parentNode]
        );
    }
    ...
    function _saveLabel(
        ...
        bytes memory name = _addLabel(label, names[parentNode]);
        names[node] = name;
        string memory labelName  = string(abi.encodePacked(label, nameFromHash[parentNode]));
        bytes32 hashLabelName = namehash(labelName);
        nameFromHash[hashLabelName] = labelName;
        return name;
    }
    ...
    function _wrapETH2LD(
        ...
        bytes memory name = _addLabel(label, "\x03mtd\x00");
        names[node] = name;
        string memory ETH2LDName  = string(abi.encodePacked(label, ".mtd"));
        bytes32 hashETH2LDName = namehash(label);
        nameFromHash[hashETH2LDName] = ETH2LDName;
        _wrap(
            node,
            name,
            wrappedOwner,
    ...
    function decodeBytes(bytes memory data) public pure returns (string memory) {
        uint8 labelLength = uint8(data[0]);
        bytes memory label = new bytes(labelLength);
        for (uint i = 0; i < labelLength; i++) {
            label[i] = data[i + 1];
        }

        bytes memory name = new bytes(data.length - 1 - labelLength);
        for (uint i = 0; i < name.length; i++) {
            name[i] = data[i + 1 + labelLength];
        }
